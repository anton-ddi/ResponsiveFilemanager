		...
		foreach ($files as $file_array) {
			...
			$file=$file_array['file'];
				...
				if(!file_exists($src_thumb)){
				    try {
					create_img_gd($file_path, $src_thumb, 160, 160);
...
?>
            ...
            ?>		<figure data-name="<?php echo $file ?>" data-type="<?php if($is_img){ echo "img"; }else{ echo "file"; } ?>">
				...
				<div class="img-precontainer-mini <?php if($is_img) echo 'original-thumb' ?>">
					...
					<span></span>
					<?php if($mini_src!=""){ ?>
					<img alt="<?php echo $filename." thumbnails";?>" class="<?php echo $show_original_mini ? "original" : "" ?> <?php echo $is_icon_thumb_mini ? "icon" : "" ?>" src="<?php echo $src_thumb; ?>">
					<?php } ?>
				    </div>
				</div>
